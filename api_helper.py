...
def get_movie_analytics_by_day(filters):
    int_filter_map={}
    str_filter_map={}
    ...
    for row in resultset:
        ...
        show_list=day_show_map.get(key)
        if show_list is None:
            show_list=[]
        show_list.append(show)
        day_show_map[key]=show_list

    
    day_movie_map={}

    for day in day_show_map.keys():
        show_list=day_show_map[day]
        shows=0
        ...
        day_key=date = datetime.strptime(day, '%Y%m%d').strftime('%d/%m/%Y')
        day_movie_map[day_key]=movie_perf_analytics.to_json()
    return day_movie_map
